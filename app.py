...
def upload_to_google_drive(arquivo, nome_arquivo, folder_id):
    """Faz upload de arquivo para o Google Drive - APENAS 3 ARGUMENTOS"""
    temp_path = None
    try:
        ...
        elif nome_arquivo.lower().endswith('.jpg') or nome_arquivo.lower().endswith('.jpeg'):
...
@login_required
def gerar_relatorio():
    """Gera o relatório em HTML (via AJAX)"""
    try:
        ...
        if tipo == 'presenca':
            ...
            html = gerar_relatorio_aniversarios(cursor, apenas_ativos)
        elif tipo == 'aniversarios_familiares':
            html = gerar_relatorio_aniversarios_familiares(cursor)
...
def gerar_relatorio_presenca(cursor, data_inicio, data_fim, reuniao_id, grau, loja, apenas_ativos):
    ...
    query = """
        SELECT 
            ...
            u.grau_atual,
            u.loja_nome,
            COUNT(DISTINCT pr.reuniao_id) as total_reunioes,
            COUNT(CASE WHEN pr.presente = TRUE THEN 1 END) as presentes,
            ROUND(COUNT(CASE WHEN pr.presente = TRUE THEN 1 END) * 100.0 / NULLIF(COUNT(DISTINCT pr.reuniao_id), 0), 1) as percentual
        FROM usuarios u
        LEFT JOIN presenca_reuniao pr ON u.id = pr.obreiro_id
        LEFT JOIN reunioes r ON pr.reuniao_id = r.id
        WHERE u.tipo IN ('obreiro', 'sindicante', 'admin')
    """
    ...
    if reuniao_id:
        query += " AND pr.reuniao_id = %s"
        params.append(reuniao_id)
    
    query += " GROUP BY u.id, u.nome_completo, u.grau_atual, u.loja_nome ORDER BY u.nome_completo"
    
    cursor.execute(query, params)
    resultados = cursor.fetchall()
    
    if not resultados:
        return '<div class="text-center text-muted py-4">Nenhum resultado encontrado</div>'
    ...
    for r in resultados:
        ...
        if percentual >= 75:
            ...
            status_badge = '<span class="badge" style="background: #f59e0b; padding: 4px 12px; border-radius: 20px; font-size: 0.75rem; color: white;">Atenção</span>'
        else:
        ...
        if r['total_reunioes'] == 0:
            ...
            percentual_display = f"{percentual}%"
        ...
        html += f"""
            <tr>
                ...
                <td>{r['total_reunioes']}</td>
                <td>{r['presentes']}</td>
                <td>{percentual_display}</td>
                <td>{status_badge}</td>
            </tr>
        """
        ...
        if tipo == 'presenca':
            query = """
                SELECT 
                    ...
                    u.grau_atual as grau,
                    u.loja_nome as loja,
                    COUNT(DISTINCT pr.reuniao_id) as total_reunioes,
                    COUNT(CASE WHEN pr.presente = TRUE THEN 1 END) as presentes,
                    CASE 
                        WHEN COUNT(DISTINCT pr.reuniao_id) = 0 THEN 0
                        ELSE ROUND(COUNT(CASE WHEN pr.presente = TRUE THEN 1 END) * 100.0 / COUNT(DISTINCT pr.reuniao_id), 1)
                    END as percentual
                FROM usuarios u
                LEFT JOIN presenca_reuniao pr ON u.id = pr.obreiro_id
                LEFT JOIN reunioes r ON pr.reuniao_id = r.id
    ...
    try:
        ...
        if request.method == "POST":
            ...
            try:
                ...
                registrar_log("editar", "reuniao", id, dados_anteriores=dict(reuniao_atual),
...
def documentos_candidato(id):
    ...
    try:
        ...
        if session.get('tipo') not in ['admin', 'sindicante']:
            token = request.args.get('token')
            if not token or candidato['token_acesso'] != token:
                flash("Acesso negado!", "danger")
                return redirect("/candidatos")
        ...
        cursor.execute("""
            ...
            WHERE ativo = 1 
            ORDER BY obrigatorio DESC, ordem
        """)
        tipos_documentos = cursor.fetchall()
        ...
        # Criar mapa de documentos por tipo
        documentos_map = {}
        ...
        tipos_obrigatorios = [t for t in tipos_documentos if t['obrigatorio'] == 1]
        ...
        for tipo in tipos_obrigatorios:
            doc = documentos_map.get(tipo['id'])
            if doc and doc['status'] != 'rejeitado':
                enviados_obrigatorios += 1
            if doc and doc['status'] == 'pendente':
                documentos_pendentes += 1
        
        # Documentos opcionais (apenas para contagem)
        tipos_opcionais = [t for t in tipos_documentos if t['obrigatorio'] == 0]
        documentos_opcionais_count = len(tipos_opcionais)
        
        # Calcular percentual
        percentual = int((enviados_obrigatorios / total_obrigatorios * 100)) if total_obrigatorios > 0 else 0
        
        print(f"📊 Documentos do candidato {candidato['nome']}:")
        ...
        return render_template("documentos_candidato.html",
                              candidato=candidato,
                              tipos_documentos=tipos_documentos,
                              documentos_map=documentos_map,
                              total_obrigatorios=total_obrigatorios,
