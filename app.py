...
@app.route("/migrar-foto")
def migrar_foto():
    """Rota temporária para adicionar coluna foto na tabela usuarios"""
    try:
        cursor, conn = get_db()
        ...
        cursor2.execute("SELECT COUNT(*) as total FROM usuarios WHERE foto IS NOT NULL")
        total_com_foto = cursor2.fetchone()['total']
        cursor2.execute("SELECT COUNT(*) as total FROM usuarios")
        total_usuarios = cursor2.fetchone()['total']
            ...
            <div class="container">
                ...
                <div class="info">
                    ...
                    <ul style="margin-top: 10px;">
                        <li>✅ Coluna <code>foto</code> adicionada à tabela <code>usuarios</code></li>
                        <li>✅ Campo para armazenar o nome do arquivo da foto</li>
                        <li>✅ Pronto para upload de imagens</li>
                    </ul>
                </div>
                
                <div class="stats">
                    <div class="number">{total_usuarios}</div>
                    <div>Total de obreiros cadastrados</div>
                    <div class="number mt-3">{total_com_foto}</div>
                    <div>Obreiros com foto</div>
                </div>
                
                <div class="info">
                    <strong>📸 Como usar:</strong>
                    <ol style="margin-top: 10px;">
                        <li>Acesse a página de edição de um obreiro</li>
                        <li>Na seção "Foto do Obreiro", clique em "Escolher arquivo"</li>
                        <li>Selecione uma imagem (PNG, JPG, JPEG, GIF, WEBP)</li>
                        <li>Clique em "Upload Foto"</li>
                        <li>A imagem será redimensionada automaticamente para 300x300 pixels</li>
                    </ol>
                </div>
                
                <div style="text-align: center; margin-top: 30px;">
                    <a href="/obreiros" class="btn btn-success">Ir para Obreiros</a>
                    <a href="/dashboard" class="btn btn-primary">Voltar ao Dashboard</a>
                </div>
                
                <hr>
                
                <div class="alert alert-warning" style="background: #fff3cd; padding: 15px; border-radius: 10px; margin-top: 20px;">
                    <strong>⚠️ Importante:</strong> Esta é uma rota temporária de migração.
                    Após confirmar que a coluna foi criada, remova a rota <code>/migrar-foto</code> do código.
                </div>
            </div>
        </body>
        </html>
